-- Hard, server-side guarantee against duplicate requirements — closes the
-- gap for anything that gets past the client-side double-submit guard
-- (multiple tabs open at once, a network-level retry, etc.). The teacher
-- side doesn't need an equivalent: teacher_profiles.user_id and
-- teacher_languages(teacher_id, language) are already unique-constrained
-- in 0001_init_schema.sql, so a repeat submission there just re-saves the
-- same row rather than creating a duplicate.
--
-- If an OPEN requirement already exists for this exact parent + student +
-- subject, return that row instead of inserting a second one. Only checks
-- 'open' status deliberately — once a requirement has been assigned, a
-- fresh request for the same subject (e.g. needing a new tutor) is a
-- legitimately different case and should be allowed to create a new row.
...
begin
  ...
  select * into v_req from requirements
  where parent_id = p_parent_id
    and subject = p_subject
    and student_id is not distinct from v_student.id
    and status = 'open'
  limit 1;
