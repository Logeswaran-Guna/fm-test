    ...
    <div className="flex gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: 5 }, (_, i) => (
        <svg
          ...
          width="15"
          height="15"
          fill={i < rating ? (dark ? "#4FE5D7" : "#0f7c6c") : "none"}
  ...
  return (
    <div
      className={`relative flex h-full flex-col overflow-hidden rounded-2xl border p-6 shadow-sm sm:p-7 ${
        dark ? "border-navy bg-navy" : "border-slate-200 bg-white"
      }`}
    >
      ...
      <div className="pointer-events-none absolute right-5 top-5 grid grid-cols-3 gap-1" aria-hidden>
        {Array.from({ length: 9 }, (_, i) => (
          <span key={i} className={`h-1 w-1 rounded-full ${dark ? "bg-[#4FE5D7]/40" : "bg-amber/40"}`} />
        ))}
      </div>
      ...
      <svg
        className={`relative z-10 mt-4 h-7 w-7 ${dark ? "text-[#4FE5D7]/70" : "text-amber-500/70"}`}
        viewBox="0 0 24 24"
        fill="currentColor"
      >
      ...
      <div className="relative z-10 mt-5">
        <Stars rating={testimonial.rating} dark={dark} />
...
export default function Testimonials() {
  ...
  return (
    <section className="bg-[#EAF7F4] pt-8 pb-12 sm:pt-10 sm:pb-16">
      <div className="mx-auto max-w-7xl px-6 sm:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <TestimonialCard key={t.id} testimonial={t} dark={i % 3 === 1} />
          ))}
        </div>
      </div>
    </section>
